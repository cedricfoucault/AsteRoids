...
- (void)setupParticles {
    ...
    for(int i=0; i<N_PARTICLES_MAX; i++) {
        // Randomize position
        NGLvec4 particleCenter = nglVec4Make(0, 0, 0, 1.0);
//        particleCenter.x = ((float)arc4random_uniform(RAND_MAX) / (float)RAND_MAX - 0.5) * WINDOW_SCALE;
//        particleCenter.y = ((float)arc4random_uniform(RAND_MAX) / (float)RAND_MAX - 0.5) * WINDOW_SCALE / WINDOW_ASPECT_RATIO;
        
        // Set up geometry
        _quads[i].bl.vertex.x = particleCenter.x - PARTICLE_SIZE / 2;
        _quads[i].bl.vertex.y = particleCenter.y - PARTICLE_SIZE / 2;
        _quads[i].bl.vertex.z = particleCenter.z;
        _quads[i].bl.vertex.w = particleCenter.w;
        
        _quads[i].br.vertex.x = particleCenter.x + PARTICLE_SIZE / 2;
        _quads[i].br.vertex.y = particleCenter.y - PARTICLE_SIZE / 2;
        _quads[i].br.vertex.z = particleCenter.z;
        _quads[i].br.vertex.w = particleCenter.w;
        
        _quads[i].tr.vertex.x = particleCenter.x + PARTICLE_SIZE / 2;
        _quads[i].tr.vertex.y = particleCenter.y + PARTICLE_SIZE / 2;
        _quads[i].tr.vertex.z = particleCenter.z;
        _quads[i].tr.vertex.w = particleCenter.w;
        
        _quads[i].tl.vertex.x = particleCenter.x - PARTICLE_SIZE / 2;
        _quads[i].tl.vertex.y = particleCenter.y + PARTICLE_SIZE / 2;
        _quads[i].tl.vertex.z = particleCenter.z;
        _quads[i].tl.vertex.w = particleCenter.w;
